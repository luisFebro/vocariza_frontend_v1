...
export default function isMoneyBrValidAndAlert(string, showSnackbar, dispatch) {
    ...
    if (commaLength > 1)
        return showSnackbar(
            dispatch,
            "Insira apenas uma vírgula por valor",
            "error"
        );
    ...
    if (endValue === ",")
        return showSnackbar(
            dispatch,
            "Você digitou um número com vírgula sem decimal. Retire a vírgula ou acrescente valor decimal",
            "error",
            8000
        );
    if (["0,0", "", null].includes(string))
        return showSnackbar(dispatch, "O valor não pode ser zero", "error");
    if (["null", "undefined", "NaN"].includes(string))
        return showSnackbar(
            dispatch,
            "Campo com erros ou vazio. Digite apenas valores numéricos com/sem vírgula",
            "error"
        );
    ...
    if (isNaN(parseFloat(string)))
        return showSnackbar(
            dispatch,
            "Ops! Parece que você não digitou um formato numérico",
            "error"
        );
    return true;
}
